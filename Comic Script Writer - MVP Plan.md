...
**Second round of layout/interaction fixes:**
- **Pages are physical 8.5×11in sheets** with 1in margins, centered, so the writing area no longer grows and shrinks with content. A comic page whose content genuinely exceeds one sheet extends past 11in rather than being clipped — true mid-node pagination (reflowing a panel across a page break) is a substantially larger feature and is **not** in the MVP. Worth knowing: a "page" here is a *comic* page, which in the reference PDF happens to be one printed sheet each, but nothing enforces that as content grows.
- **New dialogue lines land in the character field**, prefilled with the previous speaker and fully selected — so typing replaces it for a new speaker, or Tab keeps it for the same one. Previously a new line silently inherited the previous character with no chance to change it without the mouse.
- **Inline name autocomplete**: typing matches against the project cast and completes with the suggested tail selected; Tab accepts it and moves into the line. This replaces the native `<datalist>` dropdown, which couldn't be driven by keyboard alone. Detecting "is the user inserting or deleting?" uses the input event's `inputType` rather than comparing string lengths — length comparison misreads typing over the selected prefilled name (the value gets *shorter*) as a deletion and suppresses the suggestion on exactly the keystroke where it's most useful.
- **The colon tracks the name's width.** It's positioned by measuring a hidden copy of the text rather than a `ch`-based estimate, because Verdana is proportional and `ch` units left the colon floating away from short names. The 2in tab stop for the dialogue text itself is unaffected.
- **`Ctrl/Cmd+Backspace` deletes the current panel outright**, dialogue and all. This fixes a real gap: plain Backspace only removes lines that are *already empty*, and the old `Mod-Backspace` also required an empty panel, so a panel with text in it could not be removed at all. If the panel is its page's only content the page goes with it, unless it's the last page.
